  ...
  void populateChapters(
    ...
    for (int i = 0; i < chapterList.length; i++) {
      ...
      for (int j = 0; j < readReferenceList.length; j++) {
        //  print('(D69-30)${readReferenceList[j].chapter}*${chapter.ffRef}');
        if (readReferenceList[j].chapter == chapter.reference) {
          ...
          if ((FFAppState().chosenColors == null) ||
              (FFAppState().chosenColors.length < 1)) {
            FFAppState().chosenColors = List.from(defaultColors);
          }
